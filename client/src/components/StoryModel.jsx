...
const StoryModel = ({ setShowModal, fetchStories }) => {
    ...
    const bgColor = ["#4f4635", "#7c3f2d", "#db4d3b", "#e11381", "#ca4c3a", "#0d2e29"];
            ...
            <div className='w-full max-w-md'>
                ...
                <div className='flex gap-2 mt-4'>
                    <button onClick={() => {setMode('text'); setMedia(null); setPreviewUrl(null)}}
                        className={`flex-1 flex items-center justify-center gap-2 rounded cursor-pointer ${mode === 'text' ? 'bg-white text-black' : 'bg-zinc-800'}`}
                        >
                        <TextIcon size={18} /> Text
                    </button>
                    <label className={`flex-1 flex items-center justify-center gap-2 p-2 rounded cursor-pointer ${mode === "media" ? "bg-white text-white" : "bg-zinc-800" }`}>
